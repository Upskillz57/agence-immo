...
import { LayoutGrid, Map, SlidersHorizontal, Home, BedDouble, Maximize } from "lucide-react";
...
useEffect(() => {
    fetch("/api/properties")
      .then(res => res.json())
      .then(data => {
        console.log("PREMIER BIEN:", data[0]);
        console.log("PRIX EXEMPLES:", data.slice(0,5).map((p:any) => ({ title: p.title, price: p.price })));
        setProperties(data);
      });
  }, []);
...
const filteredProperties = Array.isArray(properties)
  ? properties.filter((property) => {
      if (filters.amenities.length > 0) {
        const hasAmenity = filters.amenities.every(a => property.amenities?.includes(a));
        if (!hasAmenity) return false;
      }
      if (filters.type !== "Tous" && !property.type?.toLowerCase().includes(filters.type.toLowerCase())) return false;
...
const isActive = (value: string) => filters.amenities.includes(value);
    ...
    <div className="hidden md:flex flex-row gap-3 mb-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="hidden md:flex flex-wrap gap-3">
          ...
          <button onClick={() => setIsFilterOpen(true)} className="hidden md:flex items-center gap-2 border px-4 py-2 rounded-full text-sm bg-white text-gray-900 hover:border-black">
          ...
          <div className="relative">
            ...
            {openDropdown === "bedrooms" && (
              <div className="absolute top-full mt-2 bg-white text-gray-900 border shadow-xl w-[200px] z-50">
                {["1","2","3","4"].map(item => (
                  <div key={item} onClick={() => { setFilters(prev => ({ ...prev, bedrooms: item })); setOpenDropdown(null); }} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">{item}+</div>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            {openDropdown === "surface" && (
              <div className="absolute top-full mt-2 bg-white text-gray-900 border shadow-xl w-[200px] z-50">
                {["50","100","150"].map(item => (
                  <div key={item} onClick={() => { setFilters(prev => ({ ...prev, surface: item })); setOpenDropdown(null); }} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">{item}+ m²</div>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            {openDropdown === "amenities" && (
              <div className="absolute top-full mt-2 bg-white text-gray-900 border shadow-xl w-[260px] z-50 p-3 flex flex-wrap gap-2">
                {["Piscine","Terrasse","Jardin","Parking","Clim","Garage","Sauna"].map(item => (
          ...
          <div className="relative">
            ...
            {openDropdown === "bathrooms" && (
              <div className="absolute top-full mt-2 bg-white text-gray-900 border shadow-xl w-[200px] z-50">
                {["1","2","3","4"].map(item => (
                  <div key={item} onClick={() => { setFilters(prev => ({ ...prev, bathrooms: item })); setOpenDropdown(null); }} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">{item}+</div>
                ))}
              </div>
            )}
          </div>
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        {filters.type !== "Tous" && (
          <div className="px-3 py-1 bg-[#122e53] text-white text-xs rounded-full cursor-pointer" onClick={() => setFilters(prev => ({ ...prev, type: "Tous" }))}>{filters.type} ✕</div>
        )}
        {filters.transaction && (
          <div className="px-3 py-1 bg-[#122e53] text-white text-xs rounded-full cursor-pointer" onClick={() => setFilters(prev => ({ ...prev, transaction: "" }))}>{filters.transaction === "vente" ? "À vendre" : "À louer"} ✕</div>
        )}
        {filters.bedrooms && (
          <div className="px-3 py-1 bg-[#122e53] text-white text-xs rounded-full cursor-pointer" onClick={() => setFilters(prev => ({ ...prev, bedrooms: "" }))}>{filters.bedrooms} chambres ✕</div>
        )}
        {filters.bathrooms && (
          <div className="px-3 py-1 bg-[#122e53] text-white text-xs rounded-full cursor-pointer" onClick={() => setFilters(prev => ({ ...prev, bathrooms: "" }))}>{filters.bathrooms} SDB ✕</div>
        )}
        {filters.surface && (
          <div className="px-3 py-1 bg-[#122e53] text-white text-xs rounded-full cursor-pointer" onClick={() => setFilters(prev => ({ ...prev, surface: "" }))}>{filters.surface} m² ✕</div>
        )}
        {filters.amenities.map(item => (
          <div key={item} onClick={() => setFilters(prev => ({ ...prev, amenities: prev.amenities.filter(a => a !== item) }))} className="px-3 py-1 bg-[#122e53] text-white text-xs rounded-full cursor-pointer">{item} ✕</div>
        ))}
      </div>
        ...
        <div className="flex justify-center w-full bg-[#f5f5f5] py-6">
          <div className="w-full max-w-[1200px] flex gap-6">
            <div className="flex-1">
              <PropertyList properties={filteredProperties} hoveredId={hoveredId} setHoveredId={setHoveredId} />
            </div>
        ...
        <div className="flex w-full h-[calc(100vh-140px)] overflow-hidden">
          ...
          <div className="w-[320px] min-w-[320px] bg-white border-r overflow-y-auto">
            ...
            <div className="px-3 pt-3">
              <PropertyListMap properties={filteredProperties} hoveredId={hoveredId} setHoveredId={setHoveredId} />
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40" onClick={() => setIsFilterOpen(false)}>
      <div className="bg-white w-full max-w-[900px] mt-10 shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 text-[#122e53] overflow-y-auto flex-1">
          ...
          {/* RECHERCHE VILLE — mobile uniquement */}
<div className="md:hidden py-4 border-b">
  <div className="flex items-center gap-3 mb-3">
    <Home size={18} />
    <span className="font-medium">Ville / Code postal</span>
  </div>
  <input
  ...
  />
          ...
          <div className="relative py-4 border-b">
            <div onClick={() => toggleDropdown("transaction")} className="flex items-center justify-between cursor-pointer">
              ...
              <div className="flex items-center gap-2 text-gray-500"><span>{filters.transaction === "vente" ? "À vendre" : "À louer"}</span><span>▾</span></div>
            </div>
            {openDropdown === "transaction" && (
              <div className="absolute right-0 mt-2 w-[220px] bg-white border shadow-xl z-50">
                <div onClick={() => setTransaction("vente")} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">À vendre</div>
                <div onClick={() => setTransaction("location")} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">À louer</div>
              </div>
            )}
          </div>
          ...
          <div className="relative py-4 border-b">
            ...
            {openDropdown === "type" && (
              <div className="absolute right-0 mt-2 w-[240px] bg-white border shadow-xl z-50">
                {["Tous","Maison","Appartement","Terrain"].map(item => (
                  <div key={item} onClick={() => setType(item)} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">{item}</div>
                ))}
              </div>
          ...
          <div className="py-4 border-b">
            ...
            <div className="flex flex-wrap gap-2">
              {["Piscine","Meublé","Terrasse","Ascenseur","Climatisation","Jardin","Balcon","Parking"].map(item => (
                <button key={item} onClick={() => toggleAmenity(item)} className={`px-3 py-2 border text-sm transition ${isActive(item) ? "bg-[#122e53] text-white border-[#122e53]" : "bg-white hover:bg-gray-100"}`}>{item}</button>
              ))}
            </div>
          </div>

          <div className="py-4 border-b">
            <div className="flex items-center gap-3 mb-4"><Maximize size={18} /><span className="font-medium">Prix</span></div>
            <input type="range" min="0" max="1000000" step="50000" value={filters.priceMax || 1000000} onChange={e => setFilters(prev => ({ ...prev, priceMax: e.target.value }))} className="w-full accent-[#122e53]" />
            <div className="flex justify-between text-sm text-gray-500 mt-2"><span>0€</span><span>{filters.priceMax || "1 000 000"} €</span></div>
          </div>

          <div className="relative py-4 border-b">
            <div onClick={() => toggleDropdown("surface")} className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center gap-3"><Maximize size={18} /><span className="font-medium">Surface</span></div>
              <div className="flex items-center gap-2 text-gray-500"><span>{filters.surface || "Peu importe"}</span><span>▾</span></div>
            </div>
            {openDropdown === "surface" && (
          ...
          <div className="relative py-4 border-b">
            <div onClick={() => toggleDropdown("bathrooms")} className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center gap-3"><BedDouble size={18} /><span className="font-medium">Salle de bain</span></div>
              <div className="flex items-center gap-2 text-gray-500"><span>{filters.bathrooms || "Peu importe"}</span><span>▾</span></div>
            </div>
            {openDropdown === "bathrooms" && (
              <div className="absolute right-0 mt-2 w-[200px] bg-white border shadow-xl z-50">
                {["1+","2+","3+","4+"].map(item => (
          ...
          <div className="relative py-4 border-b">
            <div onClick={() => toggleDropdown("bedrooms")} className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center gap-3"><BedDouble size={18} /><span className="font-medium">Chambres</span></div>
              <div className="flex items-center gap-2 text-gray-500"><span>{filters.bedrooms || "Peu importe"}</span><span>▾</span></div>
            </div>
            {openDropdown === "bedrooms" && (
              <div className="absolute right-0 mt-2 w-[200px] bg-white border shadow-xl z-50">
                {["1+","2+","3+","4+"].map(item => (
        ...
        <div className="flex justify-between items-center p-4 border-t">
          <button onClick={() => setFilters({ amenities: [], type: "Tous", transaction: "vente", priceMax: "", bedrooms: "", bathrooms: "", surface: "", location: "" })} className="text-gray-500 hover:text-black">Réinitialiser</button>
          <button onClick={() => setIsFilterOpen(false)} className="bg-[#122e53] hover:bg-[#0f2442] transition text-white px-6 py-2 rounded-full">Voir les biens</button>
        </div>
